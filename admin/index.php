...
$recentPosts = db_all(
    'SELECT p.id, p.body, p.created_at, t.id AS topic_id, t.title, u.username
       FROM posts p
    ...
    foreach ($cards as $card): ?>
        <article class="panel p-4">
            <div class="flex items-center justify-between">
                <span class="text-[10px] uppercase tracking-[0.2em] text-ink-soft"><?= e($card['label']) ?></span>
            ...
            <p class="mt-1 text-xs text-ink-soft"><?= e($card['note']) ?></p>
        </article>
    <?php endforeach; ?>
</div>
...
<div class="mt-6 grid gap-6 xl:grid-cols-2">
        ...
        <ul class="divide-rule">
            <?php foreach ($openReports as $report): ?>
                <li class="px-4 py-3">
                    ...
                    <p class="mt-0.5 text-xs text-ink-soft"><?= e(excerpt((string) $report['reason'], 120)) ?></p>
            ...
            <?php endforeach; ?>
            <?php if ($openReports === []): ?>
                <li class="px-4 py-8 text-center text-sm italic text-ink-soft">Nothing is waiting. The hall is calm.</li>
            <?php endif; ?>
        </ul>
        <div class="border-t border-rule p-3">
            <a class="btn btn-ghost btn-sm" href="<?= e(url('admin/reports.php')) ?>">Open the report queue</a>
        </div>
    </section>
    ...
    <section class="panel">
        ...
        <ul class="divide-rule">
            <?php foreach ($recentMembers as $member): ?>
                <li class="flex items-center justify-between gap-3 px-4 py-2.5">
                    <div class="min-w-0">
                        <a class="row-link text-sm" href="<?= e(url('profile.php?id=' . (int) $member['id'])) ?>"><?= e((string) $member['username']) ?></a>
                        <p class="text-[11px] text-ink-soft">joined <?= e(time_ago((string) $member['created_at'])) ?></p>
                    </div>
        ...
        <div class="border-t border-rule p-3">
            <a class="btn btn-ghost btn-sm" href="<?= e(url('admin/users.php')) ?>">Manage members</a>
        </div>
    </section>
        ...
        <div class="divide-rule">
            <?php foreach ($recentPosts as $post): ?>
                <article class="px-4 py-3">
                    ...
                    <p class="mt-1 text-xs text-ink-soft"><?= e(excerpt((string) $post['body'], 170)) ?></p>
                </article>
            <?php endforeach; ?>
            <?php if ($recentPosts === []): ?>
                <p class="px-4 py-8 text-center text-sm italic text-ink-soft">No posts yet.</p>
            <?php endif; ?>
        </div>
    </section>
</div>
