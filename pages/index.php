...
$boards = db_all(
    'SELECT b.id, b.slug, b.category_id, b.name, b.description, b.icon, b.is_locked,
...
$latestTopics = db_all(
    'SELECT t.id, t.title, t.slug, t.last_post_at, t.reply_count, b.name AS board_name, u.username
       FROM topics t
...
<section class="panel mb-6 overflow-hidden">
    <div class="relative">
        ...
        <div class="absolute inset-0 flex flex-col justify-center px-5 sm:px-8">
            ...
            <p class="mt-1 max-w-md text-sm text-soft">
    ...
    <div class="grid grid-cols-2 divide-x divide-rule border-t border-rule text-center sm:grid-cols-4">
        ...
        foreach ($tiles as $tile): ?>
            <div class="px-3 py-3">
                ...
                <div class="text-[10px] uppercase tracking-[0.2em] text-soft"><?= e($tile['label']) ?></div>
            </div>
        <?php endforeach; ?>
    </div>
</section>
...
<div class="grid gap-6 lg:grid-cols-[minmax(0,1fr)_18rem]">
    <div class="space-y-6">
        <?php if ($categories === []): ?>
            <div class="panel p-8 text-center text-sm text-soft">
        ...
        <?php foreach ($categories as $category): ?>
            ...
            <section class="panel">
                ...
                <?php if ((string) $category['description'] !== ''): ?>
                    <p class="border-b border-rule bg-parchment-dark px-4 py-2 text-xs text-soft">
                ...
                <div class="divide-rule">
                    <?php if ($catBoards === []): ?>
                        <p class="px-4 py-5 text-sm text-soft">This category has no boards yet.</p>
                    <?php endif; ?>
                        ...
                        <article class="flex flex-wrap items-center gap-4 px-4 py-3 transition-colors hover:bg-parchment-dark/50">
                            ...
                            <div class="min-w-0 flex-1">
                                <h3 class="text-[15px] font-semibold">
                                    <a class="row-link" href="<?= e(board_url((string) $board['slug'])) ?>">
                                        <?= e((string) $board['name']) ?>
                                    </a>
                                ...
                                <p class="mt-0.5 text-xs leading-relaxed text-soft"><?= e((string) $board['description']) ?></p>
                            </div>

                            <dl class="hidden w-28 shrink-0 text-center text-xs text-soft sm:block">
                            ...
                            <div class="hidden w-56 shrink-0 border-l border-rule pl-4 text-xs text-soft lg:block">
                                <?php if ($board['last_topic_id'] !== null): ?>
                                    <a class="row-link block truncate text-[13px]"
                                       href="<?= e(topic_url((string) $board['last_topic_slug'])) ?>">
                                        <?= e(excerpt((string) $board['last_topic_title'], 34)) ?>
                                    </a>
                                    <span class="mt-0.5 block">
                                        by
                                        <?php if ($board['last_user_id'] !== null): ?>
                                            <a class="font-medium text-ink hover:underline"
                                               href="<?= e(member_url((string) $board['last_username'])) ?>"><?= e((string) $board['last_username']) ?></a>
                                        <?php else: ?>
    ...
    <aside class="space-y-6">
        <section class="panel">
            ...
            <ul class="divide-rule">
                <?php foreach ($latestTopics as $topic): ?>
                    <li class="px-4 py-2.5">
                        <a class="row-link block text-[13px] leading-snug"
                           href="<?= e(topic_url((string) $topic['slug'])) ?>">
                ...
                <?php endforeach; ?>
                <?php if ($latestTopics === []): ?>
                    <li class="px-4 py-4 text-xs italic text-soft">Nothing has been posted yet.</li>
                <?php endif; ?>
            </ul>
        </section>
        ...
        <section class="panel">
            ...
            <dl class="space-y-2 px-4 py-3 text-xs text-soft">
            ...
            <?php if (!is_logged_in()): ?>
                <div class="border-t border-rule px-4 py-3">
                    <a href="<?= e(url('register')) ?>" class="btn btn-gold w-full">
